...
function drawWall(doc: jsPDF, el: WallElevation, opts: PdfOptions, backgrounds: PdfBackground[]): void {
  ...
  doc.setFontSize(13);
  // Fotostěna má jediný líc — označení strany by jen mátlo.
  const title = el.wall.photoWall
...
function drawBlock(doc: jsPDF, el: WallElevation, denom: number): void {
  ...
  doc.setFontSize(8);
  doc.text(el.wall.photoWall ? `${el.storeyName} — ${el.wall.name}` : `${el.storeyName} — ${el.wall.name} (${el.side})`, bx + 3, by + bh - 6.5);
  doc.setFontSize(7);
